...
const fetchWithAuth = async (url: string, options: RequestInit = {}) => {
    ...
    if (response.status === 401) { // Token is invalid or expired
        ...
        // Instead of a hard reload which causes a loop, dispatch a custom event.
        // The main App component will listen for this to gracefully log the user out.
...
const App: React.FC = () => {
  ...
  const handleLogout = useCallback(() => {
    ...
    isDataLoaded.current = false; // Reset data loaded flag for next login
  }, []);
  ...
  const loadAppData = useCallback(async () => {
    try {
        ...
        if (!(e.message.includes('Session expired'))) {
          setFinalizeError(e.message || 'An unexpected error occurred during data loading.');
        }
    }
  }, []);
  ...
  // Effect to handle graceful logout on session expiration
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
        ...
        if (!savedCompanyName) {
            if (isMounted) setAppState('create_profile');
            return;
        }
        if (isMounted) setCompanyName(savedCompanyName);
        ...
        try {
            if (code) {
                ...
                if (isMounted) setAppState('logged_out');
            }
        } catch (error: any) {
  ...
  // Decouple data loading from authentication. Load data only when app is in the 'main' state.
  useEffect(() => {
    if (appState === 'main' && !isDataLoaded.current) {
  ...
  const renderContent = () => {
    switch(appState) {
        ...
        case 'logged_out':
            return <Login apiBaseUrl={API_BASE_URL} authError={authError} />;
        case 'error':
             return (
