...
export const Login: React.FC<LoginProps> = ({ apiBaseUrl, authError }) => {
    const [clientId, setClientId] = useState<string | null>(null);
    const [connectionError, setConnectionError] = useState('');

    useEffect(() => {
        // This is the most common deployment error. Provide a clear, actionable message.
        // Check if the app is deployed (not on localhost) but is still trying to use the localhost backend URL.
        ...
        if (isProduction && (!apiBaseUrl || apiBaseUrl.includes('localhost'))) {
            setConnectionError('Configuration Error: The frontend is deployed, but it is trying to connect to a local backend. Please set the VITE_API_BASE_URL environment variable in your hosting settings to your live backend URL.');
        ...
        const fetchClientId = async () => {
            try {
                ...
                if (!response.ok) throw new Error(`Could not fetch Google Client ID. Status: ${response.status}`);
                const data = await response.json();
                if (!data.clientId) throw new Error('Google Client ID is missing from the server response.');
                setClientId(data.clientId);
            } catch (err: any) {
                console.error(err); // Log the actual error for debugging
                setConnectionError(`Could not connect to the server to start login process. Please ensure the backend is running and the VITE_API_BASE_URL is correct.`);
            }
        };
        fetchClientId();
    }, [apiBaseUrl]);
    ...
    const handleLogin = () => {
        if (!clientId) {
            setConnectionError('Google Client ID is not configured on the server. Cannot initiate login.');
            return;
        }
    ...
    const displayError = authError || connectionError;
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-cream-50 dark:bg-gray-900">
            <div className="w-full max-w-sm p-8 space-y-8 bg-white dark:bg-gray-800 rounded-2xl shadow-xl text-center animate-scale-in">
                ...
                {displayError ? (
                    <div className="text-red-600 dark:text-red-400 text-sm bg-red-50 dark:bg-red-900/20 p-4 rounded-lg text-left">
                        <p className="font-bold text-red-700 dark:text-red-300">{authError ? 'Authentication Failed' : 'Connection Error'}</p>
                    ...
                    >
                        <GoogleIcon />
                        <span className="ml-3">{clientId ? 'Sign in with Google' : 'Connecting to Server...'}</span>
                    </button>
                )}
            </div>
        </div>
